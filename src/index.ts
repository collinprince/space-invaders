...
import { parseInputAndReturnAction, parseInputAndStopPlayerMoving, WorldModifierFunc } from "./input-handling";
...
function init(): WorldState {
    let world: WorldState =  {
        player: new PlayerShip(canvas.width/2 - 10, canvas.height - 100, 20, 30, 0, 0),
        gameObjects: new Array(),
    };
    for (let i = 0; i < 10; ++i) {
        world.gameObjects.push(new EnemyShip(20 + canvas.width/10 * i, canvas.height/10, 10, 10, 0, 0));
    }
    return world;
}
